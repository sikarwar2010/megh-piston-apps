    ...
    >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            key="mobile-menu"
            className="fixed inset-0 z-50 bg-background md:hidden h-screen"
                  ...
                  {navItems.map((item) => {
                    if (item.submenu) {
                      ...
                      return (
                        <div
                          key={item.path}
                          className="w-full border-b border-border/40 last:border-0 pb-2"
                        >
                          <ProductButton
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              ...
                              >
                                <div className="flex flex-col items-start space-y-2 mb-2 pl-4 border-l-2 border-primary/20 ml-1">
                                  {item.submenu.map((subItem) => (
                                    <Link
                                      key={subItem.path}
                                      href={subItem.path}
                                      onClick={() => setIsOpen(false)}
                                      className={cn(
                                        "text-base w-full py-2 transition-colors hover:text-primary",
                    ...
                    return (
                      <Link
                        ...
                        onClick={() => setIsOpen(false)}
                        className={cn(
                          "text-lg font-medium transition-colors hover:text-primary w-full py-3 border-b border-border/40 last:border-0",
                          pathname === item.path
                            ? "text-primary"
                            : "text-muted-foreground",
                        )}
